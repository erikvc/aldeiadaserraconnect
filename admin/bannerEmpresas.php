<?php
    ...
    if(isset($_GET['delCategoria'])){
        ...
        $sqlDeletaCategoria = mysqli_query($conexao, "DELETE FROM banners_empresas WHERE id = '$categoriaID'");
        $removeTemp = unlink('../slide/img/bannersEmpresas/'.$delImage);
        echo '<script>window.location.href="bannerEmpresas.php"</script>';
    }
?>
<!doctype html>
...
<section class="container-fluid">
    ...
    <h4 class="margin"><a href="newBannerEmpresas.php">Criar Novo Banner</a></h4>
...
<table class="table table-bordered">
  <thead>
    <tr>
      ...
      <th width="24" bgcolor="#F1F1F1" scope="col">Empresa</th>    
  ...
  <tbody>
      <?php
        $sqlPegaCategorias = mysqli_query($conexao, "SELECT * FROM banners_empresas ORDER BY empresa_id");  
        while($rowsCategorias = mysqli_fetch_array($sqlPegaCategorias)){
            $empresa_id = $rowsCategorias['empresa_id'];
            $pegaEmpresas = mysqli_fetch_assoc(mysqli_query($conexao, "SELECT * FROM empresas WHERE id = '$empresa_id'"));
      ?>
    <tr>
      <th bgcolor="#F7F7F7" scope="row">1</th>
      <td bgcolor="#F7F7F7" scope="row"><?php echo $pegaEmpresas['nome']; ?></td>    
      <td align="left" valign="middle" bgcolor="#F7F7F7"><img src="../slide/img/bannersEmpresas/<?php echo $rowsCategorias['imagem']; ?>" width="200" height="113" alt=""/></td>
