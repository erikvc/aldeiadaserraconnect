<?php
    ...
    if(isset($_POST['acao'])){
		...
		if (array_search($imagesType, $tiposPermitidos) != false) {
				...
				$path 		 = '../slide/img/bannersCategorias/'.$imgFileName;

				move_uploaded_file($_FILES['imagem']['tmp_name'], $path);
                
				
				$sqlPegaTasks = mysqli_query($conexao, "INSERT INTO banners_subcategoria (imagem, categoria_id)VALUES('$imgFileName', '$categoria_id')") or die(mysqli_error($conexao));
