<?php
    ...
    if(isset($_POST['acao'])){
		...
		if (array_search($imagesType, $tiposPermitidos) != false) {
				...
				$path 		 = '../optmedia/images/subcategorias/'.$imgFileName;

				move_uploaded_file($_FILES['imagem']['tmp_name'], $path);
                
				
				$sqlPegaTasks = mysqli_query($conexao, "INSERT INTO subcategorias (title, image, categoria_id)VALUES('$titulo', '$imgFileName', '$categoria_id')") or die(mysqli_error($conexao));
				
				echo '<script>window.location.href="subCategorias.php"</script>';
			}
    ...
    <div>
        <form action="" method="post" enctype="multipart/form-data">
          ...
          <div class="form-group">
              ...
              <select class="form-control" name="categoria_id" id="sel1">
                  <?php 
