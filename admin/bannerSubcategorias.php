<?php
    ...
    if(isset($_GET['delCategoria'])){
        ...
        $sqlDeletaCategoria = mysqli_query($conexao, "DELETE FROM banners_subcategoria WHERE id = '$categoriaID'");
        $removeTemp = unlink('../slide/img/bannersCategorias/'.$delImage);
        echo '<script>window.location.href="bannerSubcategorias.php"</script>';
    }
?>
<!doctype html>
...
<section class="container-fluid">
    ...
    <h4 class="margin"><a href="newBannerSubcategorias.php">Criar Novo Banner</a></h4>
...
<table class="table table-bordered">
  <thead>
    <tr>
      ...
      <th width="24" bgcolor="#F1F1F1" scope="col">Categorias</th>    
  ...
  <tbody>
      <?php
        $sqlPegaCategorias = mysqli_query($conexao, "SELECT * FROM banners_subcategoria ORDER BY categoria_id");  
        while($rowsCategorias = mysqli_fetch_array($sqlPegaCategorias)){
            $categoria_id = $rowsCategorias['categoria_id'];
            $pegaSubcategoria = mysqli_fetch_assoc(mysqli_query($conexao, "SELECT * FROM categorias WHERE id = '$categoria_id'"));
      ?>
    <tr>
      <th bgcolor="#F7F7F7" scope="row">1</th>
      <td bgcolor="#F7F7F7" scope="row"><?php echo $pegaSubcategoria['title']; ?></td>    
      <td align="left" valign="middle" bgcolor="#F7F7F7"><img src="../slide/img/bannersCategorias/<?php echo $rowsCategorias['imagem']; ?>" width="200" height="113" alt=""/></td>
